...
#include <System/bcrypt/bcrypt.h>
		...
		player_query->SetCallback([=](SqlResult *res)
		{
			if(res->Rows == 1) //login
			{
				...
				d_login->OnValidResponse([=](CPlayer *player, string inputtext)
				{
					...
					if(bcrypt_checkpw(inputtext.c_str(), sql_pass.c_str()) != 0)
					{
			...
			else //register
			{
				...
				d_register->OnValidResponse([=](CPlayer *player, string password)
				{
					...
					{
						char
							pw_salt[BCRYPT_HASHSIZE],
							pw_hash[BCRYPT_HASHSIZE + 1];

						bcrypt_gensalt(15, pw_salt);
						bcrypt_hashpw(password.c_str(), pw_salt, pw_hash);
						pw_hash[BCRYPT_HASHSIZE] = '\0';

						string password_hash(pw_hash);
